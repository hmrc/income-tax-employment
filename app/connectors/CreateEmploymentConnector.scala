...
class CreateEmploymentConnector @Inject()(val http: HttpClient,
  ...
  def createEmployment(nino: String, taxYear: Int, employment: AddEmploymentRequestModel)
    ...
    def desCall(implicit hc: HeaderCarrier): Future[CreateEmploymentResponse] = {
      http.POST[AddEmploymentRequestModel, CreateEmploymentResponse](uri, employment)
    }
