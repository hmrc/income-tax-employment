  ...
  "FakeConnector" when {
    ...
    "host is Internal" should {
      ...
      "add the correct authorization" in {
        ...
        result.authorization mustBe Some(Authorization(s"Bearer ${mockAppConfig.desAuthToken}"))
      }
      ...
      "add the correct environment" in {
        ...
        result.extraHeaders mustBe List(
          "Environment" -> mockAppConfig.desEnv
        )
      }
    }
    ...
    "host is External" should {
      ...
      "include all HeaderCarrier headers in the extraHeaders when the host is external" in {
        ...
        result.extraHeaders.contains(authorisation -> s"Bearer ${mockAppConfig.desAuthToken}") mustBe true
        result.extraHeaders.contains("Environment" -> mockAppConfig.desEnv) mustBe true
